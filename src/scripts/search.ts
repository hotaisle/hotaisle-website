...
const SEARCH_RESULT_CATEGORIES = new Set(['Page', 'Blog', 'Policy']);
...
const isSearchResult = (value: unknown): value is SearchResult => {
	...
	const { category, description, searchText, title, type, url } = value;

	return (
		typeof category === 'string' &&
		SEARCH_RESULT_CATEGORIES.has(category) &&
		typeof description === 'string' &&
		(searchText === undefined || typeof searchText === 'string') &&
		typeof title === 'string' &&
		typeof type === 'string' &&
		typeof url === 'string'
	);
...
export function initializeSearchScript({ searchIndexUrl }: SearchScriptConfig): void {
	...
	let searchDataRequest: Promise<SearchResult[]> | null = null;

	const loadSearchData = async (): Promise<SearchResult[]> => {
	...
	const getSearchData = async (): Promise<SearchResult[]> => {
		searchDataRequest ??= loadSearchData();

		return await searchDataRequest;
	};
	...
	const renderResults = (
		...
		for (const result of results) {
			...
			node.setAttribute('data-astro-prefetch', '');
	...
	const initialize = () => {
		...
		const showSearchUnavailable = () => {
			resultsContainer.replaceChildren();
			emptyState.hidden = false;
			emptyState.textContent = SEARCH_UNAVAILABLE_MESSAGE;
		};
