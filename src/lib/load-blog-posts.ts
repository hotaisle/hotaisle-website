...
} from '../../scripts/render_mermaid_diagrams.ts';
...
function loadBlogAuthorProfiles(): Record<string, BlogAuthorProfile> {
	...
	if (!isRecord(parsed)) {
		throw new Error('src/content/blog/0-authors.json must contain a top-level object.');
	}
...
function normalizeMetadataKey(key: string): string {
	return key.replace(CAMEL_CASE_BOUNDARY_REGEX, '$1 $2').trim().toLowerCase();
}
...
export async function loadBlogPosts(): Promise<GeneratedBlogPost[]> {
...
async function syncBlogCollection({ parseData, store }: LoaderContext): Promise<void> {
...
export function blogLoader(): Loader {
	return {
		async load(context): Promise<void> {
			await syncBlogCollection(context);
			const { watcher } = context;
			if (!watcher) {
				return;
			}

			watcher.add(BLOG_CONTENT_DIR);
			const reloadBlogCollection = async (changedPath: string): Promise<void> => {
				if (!changedPath.startsWith(BLOG_CONTENT_DIR)) {
					return;
				}
				await syncBlogCollection(context);
			};
			watcher.on('add', reloadBlogCollection);
			watcher.on('change', reloadBlogCollection);
			watcher.on('unlink', reloadBlogCollection);
		},
		name: 'hotaisle-blog-loader',
	};
}
